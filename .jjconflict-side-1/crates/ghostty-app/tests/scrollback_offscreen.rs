...
//!
//! Drives the real `ghostty_app::engine::Engine` (a thin wrapper over the real
//! `ghostty-vt` terminal), fills the scrollback of a small grid with uniquely
...
//! `ghostty_app::scroll` unit tests (the decision ladder + accumulator math)
//! and the windowed splits smoke; this test proves the offset actually pulls
//! the correct history rows into the rendered frame.

use ghostty_app::engine::Engine;
use ghostty_app::input::mouse::{self, MouseContext};
use ghostty_app::scroll::{ScrollMultiplier, WheelOutcome, WheelState, decide};
use ghostty_input::key_mods::Mods;
use ghostty_input::mouse::{Action, Button};
use ghostty_input::mouse_encode::MouseEvent;

/// Concatenate a snapshot row's cell characters into a trimmed string.
fn row_text(row: &ghostty_vt::snapshot::SnapshotRow) -> String {
    row.cells
        .iter()
        .map(|c| c.ch)
        .collect::<String>()
...
fn wheel_ctx(mode: MouseEvent) -> MouseContext {
    MouseContext {
        event_mode: mode,
        format: ghostty_input::mouse_encode::MouseFormat::Sgr,
