...
//!
//! When `GHOSTTY_APP_DUMP_FRAME` is set to a path prefix, the window host reads
...
//! (the IOSurface / [`ghostty_renderer::metal::Target::read_pixels`] layout);
...
impl FrameDump {
    ...
    ///
    /// `GHOSTTY_APP_DUMP_FRAME` — path prefix (enables dumping).
    /// `GHOSTTY_APP_DUMP_EVERY` — dump every Nth present (default 30, ~0.5s at
    /// 60Hz); clamped to at least 1.
    pub fn from_env() -> Option<Self> {
        let prefix = std::env::var("GHOSTTY_APP_DUMP_FRAME").ok()?;
        if prefix.is_empty() {
            return None;
        }
        let every = std::env::var("GHOSTTY_APP_DUMP_EVERY")
            .ok()
    ...
    pub fn write(&mut self, bgra: &[u8], width: usize, height: usize) {
        ...
        match encode_png_bgra(bgra, width, height) {
            Ok(png) => match std::fs::File::create(&path).and_then(|mut f| f.write_all(&png)) {
                Ok(()) => eprintln!("GHOSTTY_APP_DUMP_FRAME: wrote {path} ({width}x{height})"),
                Err(e) => eprintln!("GHOSTTY_APP_DUMP_FRAME: write {path} failed: {e}"),
            },
            Err(e) => eprintln!("GHOSTTY_APP_DUMP_FRAME: encode failed: {e}"),
        }
    }
}
