...
//! resolved [`ghostty_vt::snapshot::SnapshotCell`] view, since
...
use ghostty_vt::page::SemanticPrompt;
use ghostty_vt::snapshot::{SnapshotCell, SnapshotColor};
...
fn resolve_bg(color: SnapshotColor, palette: &ghostty_vt::color::Palette) -> Option<Rgb> {
    match color {
...
mod tests {
    use super::*;
    use ghostty_vt::snapshot::{CellStyle, CellWidth};
    ...
    fn prompt_rows_never_extend() {
        let palette = ghostty_vt::color::DEFAULT;
