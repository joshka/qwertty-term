//! Smoke tests for the libghostty-vt reference terminal.
//!
//! These only run with the `reference` feature (which links the Zig-built
//! static library): `cargo test -p vt-diff --features reference`.
...
use vt_diff::{CursorPos, Oracle, ReferenceTerminal, normalize_screen_text};

#[test]
fn hello_world_text_and_cursor() {
    let mut term = ReferenceTerminal::new(20, 5);
    term.feed(b"hello\r\nworld");
...
fn empty_terminal_dumps_empty_text() {
    let term = ReferenceTerminal::new(10, 3);
    assert_eq!(term.text(), "");
    assert_eq!(term.cursor(), CursorPos { row: 0, col: 0 });
}

/// Replay the spike's fixtures (crates/spike/tests/fixtures/replay/) through
/// the reference terminal and compare against expected.txt. Input files use
/// the `\e`-escaped convention from crates/spike/tests/replay_fixtures.rs.
#[test]
...
fn run_fixture(path: &Path) {
    ...
    let mut term = ReferenceTerminal::new(cols, rows);
...
/// Decode the `\e`-escaped fixture input convention
/// (see crates/spike/tests/replay_fixtures.rs).
