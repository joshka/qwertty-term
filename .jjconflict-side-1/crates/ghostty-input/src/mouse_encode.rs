...
//!   conversion via machinery that lives in `ghostty-vt`/the renderer. Since
//!   this crate cannot depend on `ghostty-vt`, [`Size`] is a minimal
...
//!   normally owned by `ghostty-vt`) are redefined locally as [`MouseEvent`]
...
/// on `ghostty-vt`. Variant order/names match the Zig enum (xterm mouse
...
/// depend on `ghostty-vt`. Variant order/names match the Zig enum (protocol
