...
function ghostty_restore_xdg_data_dir -d "restore the original XDG_DATA_DIR value"
    ...
    if not set -q QWERTTY_TERM_SHELL_INTEGRATION_XDG_DIR
        return
    end
    ...
    if set --function index (contains --index "$QWERTTY_TERM_SHELL_INTEGRATION_XDG_DIR" $xdg_data_dirs)
    ...
    set --erase QWERTTY_TERM_SHELL_INTEGRATION_XDG_DIR
end
...
function __ghostty_setup --on-event fish_prompt -d "Setup ghostty integration"
    ...
    set --local features (string split , $QWERTTY_TERM_SHELL_FEATURES)
    ...
    if contains path $features; and test -n "$QWERTTY_TERM_BIN_DIR"
        fish_add_path --global --path --append "$QWERTTY_TERM_BIN_DIR"
    end
    ...
    set -l features (string split ',' -- "$QWERTTY_TERM_SHELL_FEATURES")
    if contains ssh-env $features; or contains ssh-terminfo $features
        function ssh --wraps=ssh --description "SSH wrapper with Ghostty integration"
            ...
            "$QWERTTY_TERM_BIN_DIR/qwertty-term" +ssh $flags -- $argv
        end
    end
    ...
    function __ghostty_mark_prompt_start --on-event fish_prompt --on-event fish_posterror
        ...
        if test "$__qwertty_term_prompt_state" != prompt-start
            echo -en "\e]133;D\a"
        end

        set --global __qwertty_term_prompt_state prompt-start
        echo -en $__ghostty_prompt_start_mark
    end

    function __ghostty_mark_output_start --on-event fish_preexec
        set --global __qwertty_term_prompt_state pre-exec
        echo -en "\e]133;C\a"
    end

    function __ghostty_mark_output_end --on-event fish_postexec
        set --global __qwertty_term_prompt_state post-exec
