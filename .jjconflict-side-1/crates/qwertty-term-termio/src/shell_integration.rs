...
//!   integration). Preserves an existing `ZDOTDIR` into `QWERTTY_TERM_ZSH_ZDOTDIR`.
//! * **bash**: `--posix` + `ENV` trickery ([`setup_bash`]) — force POSIX-mode
//!   startup (which sources `$ENV` unconditionally) pointed at the vendored
//!   `bash/qwertty-term.bash`, which then manually replays bash's normal startup
...
//!
//! No `QWERTTY_TERM_SHELL_INTEGRATION_NO_*` variables exist at the pinned commit
//! (verified against the full upstream tree) — feature toggling is entirely
//! through the `QWERTTY_TERM_SHELL_FEATURES` comma-list env var
//! ([`setup_features`]) plus the `shell-integration = none` escape hatch
//! (skip [`setup`] entirely). The single real
//! `QWERTTY_TERM_SHELL_INTEGRATION_XDG_DIR` var is unrelated (fish/elvish/nushell
...
/// (`qwertty-term.bash`, the `zsh/` dir, ...) aren't found under `resource_dir`.
...
/// Set the `QWERTTY_TERM_SHELL_FEATURES` env var from `features`, sorted
...
pub fn setup_features(env: &mut EnvMap, features: ShellIntegrationFeatures, cursor_blink: bool) {
    ...
    if !out.is_empty() {
        env.put("QWERTTY_TERM_SHELL_FEATURES", out);
    }
}

/// Setup the bash automatic shell integration: start bash in POSIX mode and
/// use `ENV` to load the vendored `bash/qwertty-term.bash`, which then manually
...
fn setup_bash(command: &Command, resource_dir: &str, env: &mut EnvMap) -> Option<Command> {
    ...
    if let Some(v) = env.get("ENV") {
        let v = v.to_string();
        env.put("QWERTTY_TERM_BASH_ENV", v);
    }

    let script_path = format!("{resource_dir}/shell-integration/bash/qwertty-term.bash");
    if !Path::new(&script_path).is_file() {
        env.remove("QWERTTY_TERM_BASH_ENV");
        return None;
    }
    env.put("ENV", script_path);
    env.put("QWERTTY_TERM_BASH_INJECT", inject);
    if let Some(v) = rcfile {
        env.put("QWERTTY_TERM_BASH_RCFILE", v);
    }
        ...
        && let Some(home) = home_dir()
    {
        ...
        env.put("QWERTTY_TERM_BASH_UNEXPORT_HISTFILE", "1");
    }
...
fn setup_xdg_data_dirs(resource_dir: &str, env: &mut EnvMap) -> bool {
    ...
    env.put("QWERTTY_TERM_SHELL_INTEGRATION_XDG_DIR", integ_path.clone());
...
/// autoload of `nushell/vendor/autoload/qwertty-term.nu`) plus an injected
...
/// existing `ZDOTDIR` into `QWERTTY_TERM_ZSH_ZDOTDIR` so the vendored `.zshenv` can
...
fn setup_zsh(command: &Command, resource_dir: &str, env: &mut EnvMap) -> Option<Command> {
    if let Some(old) = env.get("ZDOTDIR") {
        ...
        env.put("QWERTTY_TERM_ZSH_ZDOTDIR", old);
    }
...
/// Resolve the Qwertty Term resources directory used to locate the vendored
...
///
/// Resolution order:
/// 1. `QWERTTY_TERM_RESOURCES_DIR` env var, if set and non-empty -- always wins.
...
pub fn resources_dir() -> String {
    if let Ok(dir) = std::env::var("QWERTTY_TERM_RESOURCES_DIR")
        && !dir.is_empty()
    {
        return dir;
    }
...
mod tests {
    ...
    /// `qwertty-term.bash` (setup checks the script file actually opens).
    ...
    impl TmpResourcesDir {
        fn init(shell: Shell) -> TmpResourcesDir {
            ...
            let dir = std::env::temp_dir().join(format!(
                "qwertty-term-shell-integration-test-{}-{n}",
                std::process::id()
            ));
            ...
            if shell == Shell::Bash {
                std::fs::write(shell_path.join("qwertty-term.bash"), "").unwrap();
            }
    ...
    fn setup_features_all_enabled() {
        ...
        assert_eq!(
            env.get("QWERTTY_TERM_SHELL_FEATURES"),
    ...
    fn setup_features_all_disabled() {
        ...
        assert_eq!(env.get("QWERTTY_TERM_SHELL_FEATURES"), None);
    }
    ...
    fn setup_features_mixed() {
        ...
        assert_eq!(env.get("QWERTTY_TERM_SHELL_FEATURES"), Some("ssh-env,sudo"));
    }
    ...
    fn setup_features_blinking_cursor() {
        ...
        assert_eq!(env.get("QWERTTY_TERM_SHELL_FEATURES"), Some("cursor:blink"));
    }
    ...
    fn bash_basic() {
        ...
        assert_eq!(env.get("QWERTTY_TERM_BASH_INJECT"), Some("1"));
        assert_eq!(
            env.get("ENV"),
            Some(format!("{}/qwertty-term.bash", res.shell_path.display()).as_str())
        );
    }
    ...
    fn bash_inject_flags() {
        ...
        .unwrap();
        assert_eq!(command, Command::Shell("bash --posix".to_string()));
        assert_eq!(env.get("QWERTTY_TERM_BASH_INJECT"), Some("1 --norc"));
        ...
        .unwrap();
        assert_eq!(command, Command::Shell("bash --posix".to_string()));
        assert_eq!(env.get("QWERTTY_TERM_BASH_INJECT"), Some("1 --noprofile"));
    }
    ...
    fn bash_rcfile() {
        ...
        .unwrap();
        assert_eq!(command, Command::Shell("bash --posix".to_string()));
        assert_eq!(env.get("QWERTTY_TERM_BASH_RCFILE"), Some("profile.sh"));
    ...
    fn bash_histfile() {
        ...
        // HISTFILE unset.
        {
            ...
            assert_eq!(env.get("QWERTTY_TERM_BASH_UNEXPORT_HISTFILE"), Some("1"));
        }
        ...
        // HISTFILE set.
        {
            ...
            assert_eq!(env.get("QWERTTY_TERM_BASH_UNEXPORT_HISTFILE"), None);
        }
    }
    ...
    fn bash_env() {
        ...
        let _ = setup_bash(&Command::Shell("bash".to_string()), res.path(), &mut env);
        assert_eq!(env.get("QWERTTY_TERM_BASH_ENV"), Some("env.sh"));
        assert_eq!(
            env.get("ENV"),
            Some(format!("{}/qwertty-term.bash", res.shell_path.display()).as_str())
        );
    ...
    fn bash_missing_resources() {
        let dir = std::env::temp_dir().join(format!(
            "qwertty-term-shell-integration-test-missing-{}",
            std::process::id()
        ));
    ...
    fn xdg_empty_xdg_data_dirs() {
        ...
        assert_eq!(
            env.get("QWERTTY_TERM_SHELL_INTEGRATION_XDG_DIR"),
    ...
    fn xdg_missing_resources() {
        let dir = std::env::temp_dir().join(format!(
            "qwertty-term-shell-integration-test-xdg-missing-{}",
            std::process::id()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        let mut env = EnvMap::new();
    ...
    fn nushell_unsupported_options() {
        ...
        for cmdline in cmdlines {
            ...
            assert!(env.get("QWERTTY_TERM_SHELL_INTEGRATION_XDG_DIR").is_some());
        }
    }
    ...
    fn nushell_missing_resources() {
        let dir = std::env::temp_dir().join(format!(
            "qwertty-term-shell-integration-test-nu-missing-{}",
            std::process::id()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        let mut env = EnvMap::new();
    ...
    fn zsh_basic() {
        ...
        assert_eq!(env.get("QWERTTY_TERM_ZSH_ZDOTDIR"), None);
    }
    ...
    fn zsh_zdotdir_preserved() {
        ...
        assert_eq!(env.get("ZDOTDIR"), Some(res.shell_path.to_str().unwrap()));
        assert_eq!(
            env.get("QWERTTY_TERM_ZSH_ZDOTDIR"),
            Some("$HOME/.config/zsh")
        );
    }
    ...
    fn zsh_missing_resources() {
        let dir = std::env::temp_dir().join(format!(
            "qwertty-term-shell-integration-test-zsh-missing-{}",
            std::process::id()
        ));
        std::fs::create_dir_all(&dir).unwrap();
        let mut env = EnvMap::new();
