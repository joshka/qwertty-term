//! Header drift check: regenerate `include/ghostty_rs.h` with cbindgen and diff
...
//!
//! ```sh
//! cbindgen --config crates/ghostty-ffi/cbindgen.toml \
//!   --output crates/ghostty-ffi/include/ghostty_rs.h
//! ```
//!
...
fn header_matches_checked_in() {
    let committed_path = crate_dir().join("include/ghostty_rs.h");
    let committed = std::fs::read_to_string(&committed_path)
        .expect("checked-in include/ghostty_rs.h must exist");
    ...
    if normalize(&committed) != normalize(&generated) {
        ...
        let actual_path = crate_dir().join("include/ghostty_rs.generated.h");
        let _ = std::fs::write(&actual_path, &generated);
        panic!(
            "ghostty_rs.h is out of date with the Rust C ABI. Regenerate it \
             with: cbindgen --config crates/ghostty-ffi/cbindgen.toml --output \
             crates/ghostty-ffi/include/ghostty_rs.h -- a freshly generated \
