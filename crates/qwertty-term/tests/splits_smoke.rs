...
//!
//! ```sh
//! cargo test -p qwertty-term --test splits_smoke -- --ignored --nocapture
//! ```
//!
//! Under the hood it launches the app binary with `QWERTTY_TERM_SMOKE_SPLITS=1`
//! (+ `QWERTTY_TERM_ASSERT_PRESENT=1` so each pane's presented frame is checked
...
fn windowed_splits_lifecycle() {
    let status = Command::new(BIN)
        .arg("--window")
        .env("QWERTTY_TERM_SMOKE_SPLITS", "1")
