...
//! - `QWERTTY_TERM_SMOKE_CLEARCOPY=1` — run the selection-clear-on-copy smoke:
//!   with `copy-on-select = true` + `selection-clear-on-copy = true`, assert a
//!   copy-on-select drag keeps the selection but an explicit Copy clears it,
//!   then exit 0/1.
...
//!   the resolved click interval matches, then exit 0/1.
