...
fn windowed_typing_round_trip() {
    ...
    let status = Command::new(BIN)
        .arg("--window")
        .env("GHOSTTY_APP_SMOKE_TYPE", &script)
        ...
        .env("GHOSTTY_APP_SMOKE_MS", "10000")
        .status()
    ...
    assert!(
        ...
        "windowed typing smoke failed (exit {:?}): the launched window did not \
         round-trip typed input through keyDown -> PTY -> engine. This is the \
         'renders but can't type' regression — check app activation \
         (activateIgnoringOtherApps) and first-responder wiring.",
        status.code(),
    );
}
