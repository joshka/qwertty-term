...
//!
//! ```sh
//! cargo test -p ghostty-app --test splits_smoke -- --ignored --nocapture
//! ```
//!
//! Under the hood it launches the app binary with `GHOSTTY_APP_SMOKE_SPLITS=1`
//! (+ `GHOSTTY_APP_ASSERT_PRESENT=1` so each pane's presented frame is checked
...
fn windowed_splits_lifecycle() {
    let status = Command::new(BIN)
        .arg("--window")
        .env("GHOSTTY_APP_SMOKE_SPLITS", "1")
        ...
        .env("GHOSTTY_APP_ASSERT_PRESENT", "1")
