...
pub struct Options {
    ...
    /// than `qwertty_term_vt::screen::kitty_key::Flags` directly, since this crate
    /// cannot depend on `qwertty-term-vt`; see [`KittyFlags`].
...
/// depend on `qwertty-term-vt` (whose `screen::kitty_key::FlagStack`/`Flags` own
/// the same shape, keyed by the CSI `>`/`<`/`=` `u` sequences). Callers
/// construct this from `qwertty_term_vt`'s `Flags::int()`/`from_int()` via
...
impl KittyFlags {
    ...
    /// The u5 bit representation, matching `qwertty_term_vt`'s `Flags::int()`
