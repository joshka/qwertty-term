...
//! config, restricted (for a theme) to a small key set surveyed directly
//! from the shipped themes (`Adwaita Dark`, `3024 Night`, `Aardvark Blue`,
//! `Dracula`, `Nord`, ...): `palette = N=#RRGGBB` (repeated, one per
//! index), plus `background`, `foreground`, `cursor-color`, `cursor-text`,
...
//! failing the whole file, so a theme with a key this spike doesn't apply
//! yet (or a future ghostty addition) still loads.
//!
...
/// cursor glyph color yet) — kept so the theme is fully represented and
/// future rendering work doesn't need to revisit the parser.
...
mod tests {
    ...
    // A fixture theme covering the full observed key set, modeled on the
    // real "Aardvark Blue" ghostty theme (values changed to avoid asserting
