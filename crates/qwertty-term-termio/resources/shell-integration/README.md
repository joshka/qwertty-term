...
our integration script (`bash/qwertty-term.bash`). This prevents Bash from loading
its normal startup files, which becomes our script's responsibility (along with
disabling POSIX mode).

Bash shell integration can also be sourced manually from `bash/qwertty-term.bash`.
This also works for older versions of Bash.

```bash
# Qwertty Term shell integration for Bash. This must be at the top of your bashrc!
if [ -n "${QWERTTY_TERM_RESOURCES_DIR}" ]; then
    builtin source "${QWERTTY_TERM_RESOURCES_DIR}/shell-integration/bash/qwertty-term.bash"
fi
```
...
For [Elvish](https://elv.sh), `$QWERTTY_TERM_RESOURCES_DIR/src/shell-integration`
contains an `./elvish/lib/qwertty-term-integration.elv` file.
...
with `$QWERTTY_TERM_RESOURCES_DIR/src/shell-integration`. It contains
`./elvish/lib/qwertty-term-integration.elv`. The user can then import it
by `use qwertty-term-integration` every time after shell startup or
autostart integration in `$XDG_CONFIG_HOME/elvish/rc.elv`,
which will run the integration routines.

If you decide to autostart `qwertty-term-integration` with `rc.elv`, you should
detect whether the terminal is Ghostty or not. To do this, add this to the end
of your `rc.elv` file:

```elvish
if (eq $E:TERM "xterm-ghostty") {
  try { use qwertty-term-integration } catch { }
}
```
...
```nushell
source $QWERTTY_TERM_RESOURCES_DIR/shell-integration/nushell/vendor/autoload/qwertty-term.nu
use ghostty *
```
...
```zsh
if [[ -n $QWERTTY_TERM_RESOURCES_DIR ]]; then
  source "$QWERTTY_TERM_RESOURCES_DIR"/shell-integration/zsh/qwertty-term-integration
fi
```
