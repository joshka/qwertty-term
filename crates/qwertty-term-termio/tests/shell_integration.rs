...
//! [`qwertty_term_termio::shell_integration::setup`] wired into its `Config`, feed
//! the pty output through a real `qwertty-term-vt` [`Stream`]/[`Terminal`] (the
...
use qwertty_term_termio::exec::{
    Command, Config, Exec, NullNotifier, Sink, ThreadData, WriterLoop,
};
use qwertty_term_termio::mailbox::{self};
...
fn zsh_integration_prompt_marks_and_bar_cursor() {
    ...
    assert!(
        std::path::Path::new(&resources)
            .join("shell-integration/zsh/qwertty-term-integration")
            .is_file(),
    ...
    let (waker, _wait_handle) = qwertty_term_termio::exec::CondvarWaker::new();
