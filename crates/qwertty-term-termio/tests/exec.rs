...
use qwertty_term_termio::exec::{Command, Config, Exec, Notifier, Sink, ThreadData, WriterLoop};
use qwertty_term_termio::mailbox::{self, Sender};
...
fn resize_via_mailbox_visible_to_shell() {
    ...
    tx.send(Message::Resize(qwertty_term_termio::size::Size {
        screen: ScreenSize {
            width: 1230,
            height: 820,
        },
        cell: qwertty_term_termio::size::CellSize {
            width: 10,
            height: 20,
        },
...
fn throughput_cat_10mib() {
    ...
    let (waker, _wait) = qwertty_term_termio::exec::CondvarWaker::new();
    let (tx, rx) = mailbox::channel(waker);
