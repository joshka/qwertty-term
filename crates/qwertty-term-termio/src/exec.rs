...
pub struct Config {
    ...
    /// The Qwertty Term resources dir (terminfo, XDG data, man). Optional — when
...
fn build_env(cfg: &Config, exe_dir: Option<&str>) -> Vec<(String, String)> {
    ...
    // QWERTTY_TERM_RESOURCES_DIR + TERM/COLORTERM/TERMINFO.
    if let Some(dir) = &cfg.resources_dir {
        put(&mut env, "QWERTTY_TERM_RESOURCES_DIR", dir.clone());
    ...
    // Append the ghostty bin dir to PATH (last priority) + QWERTTY_TERM_BIN_DIR.
    if let Some(exe_dir) = exe_dir {
        put(&mut env, "QWERTTY_TERM_BIN_DIR", exe_dir.to_string());
        let current = env
            .iter()
            .find(|(k, _)| k == "PATH")
            .map(|(_, v)| v.clone());
...
mod tests {
    ...
    fn build_env_with_resources_dir_sets_term_and_terminfo() {
        ...
        assert!(
            env.iter()
                .any(|(k, v)| k == "QWERTTY_TERM_RESOURCES_DIR" && v == "/app/share/ghostty")
        );
        assert!(
            env.iter()
