//! The app's binding to the real termio stack (`qwertty-term-termio`, M2 chunk E).
//!
...
use qwertty_term_termio::exec::{Command, Config, Notifier};
use qwertty_term_termio::hub::{HubHandler, Termio, Writer};
use qwertty_term_termio::shell_integration::{self, EnvMap, Shell, ShellIntegrationFeatures};
...
impl TabIo {
    ...
    pub fn spawn(
        ...
        // vendored scripts in `qwertty-term-termio/resources/shell-integration/`,
        ...
        let command = command.map(|cmd| {
            match shell_integration::setup(
                ...
                None => {
                    ...
                    // the unmodified command. QWERTTY_TERM_SHELL_FEATURES is still
        ...
        // `QWERTTY_TERM_COMMAND` is set, spawn `/bin/sh -c <value>` instead of
        ...
        let command = match std::env::var("QWERTTY_TERM_COMMAND") {
        ...
        let config = Config {
            ...
            // has already layered ZDOTDIR/QWERTTY_TERM_SHELL_FEATURES/etc. on top
            // of this same base.
