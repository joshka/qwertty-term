# Plan: M5 qwertty-term-ffi spike (start any time after M2)
...
1. **Mirror `include/ghostty.h` naming and shapes** (opaque `qwertty_term_app_t`/`surface_t`/
...
2. **cbindgen generates the header** from `crates/qwertty-term-ffi`; a CI check diffs it against
   a checked-in copy so drift is loud.
3. Spike scope (NOT the full API): `ghostty_init`, app new/free/tick, surface new/free,
   `surface_key` (one key press via qwertty-term-input), `surface_draw` into an offscreen target
