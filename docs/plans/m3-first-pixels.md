...
written — their outputs (`qwertty-term-font`, `qwertty-term-renderer` crates) are the substrate.
...
| F5-reduced CoreText face      | Opus                          | face/coretext.zig rasterization + metrics extraction; load-by-name discovery only                                                                                                                                                                                                                                            | rasterize 'A' from JetBrains Mono + a system font; alpha bitmap snapshot test; metrics parity with qwertty-term-font's table-derived values               |
...
| F6-reduced + F7-reduced       | Opus                          | single-style Collection, minimal resolver (one font + sprite dispatch to qwertty-term-sprite), run.zig reduced (single-font runs, no fallback/ligature-split), rustybuzz shaping, glyph->atlas upload via qwertty-term-font Atlas                                                                                                      | shape+rasterize "hello", an em dash, a CJK char, a symbol into atlas; positions verified                                                             |
...
| R5 window swap                | Opus                          | Thread.zig-lite (plain thread+channel first) + replace egui host with an AppKit NSWindow/NSView hosting the CALayer. INCLUDES the cheap native-shell wins: NSWindow tabbing enabled (tabbingMode; each tab = its own Engine+PTY), a minimal NSMenu (App/Edit: new tab/window, close, copy/paste, font size), Cmd-N/T/W wired | `--window` runs on Metal with native macOS tabs + menu bar; egui path kept behind a flag until parity, then deleted                                  |
...
(companion to qwertty-term-sprite), R8 shadertoy, CoreText-shaper fidelity pass.
