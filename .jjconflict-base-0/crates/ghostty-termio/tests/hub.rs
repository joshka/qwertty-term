...
//! app uses (§3.3), without pulling in `ghostty-app` (macOS-only).
...
use ghostty_termio::exec::{Command, Config, Exec, Notifier};
use ghostty_termio::hub::{HubHandler, NullHandler, Termio};
...
fn engine_sink(engine: Arc<Mutex<LiveEngine>>) -> ghostty_termio::exec::Sink {
...
/// explicitly: `cargo test -p ghostty-termio --test hub -- --ignored
/// throughput`.
#[test]
