...
//! `ghostty_vt::snapshot::SnapshotCursor`, an already-copied, borrow-free
...
//! `ghostty-vt` — see `docs/analysis/renderer-r0.md`) into `style()`'s input.

use ghostty_vt::snapshot::SnapshotCursor;
...
impl Style {
    ...
    pub fn from_terminal(term: ghostty_vt::screen::cursor::CursorStyle) -> Style {
        use ghostty_vt::screen::cursor::CursorStyle as T;
        match term {
            T::Bar => Style::Bar,
            T::Block => Style::Block,
...
/// The renderer-local cursor state that `ghostty-vt`'s snapshot doesn't
...
/// `RenderState.cursor.viewport` — `ghostty-vt` doesn't currently model
...
/// [`SnapshotCursor`] plus fields not yet wired through `ghostty-vt` (see
...
pub struct CursorState {
    ...
    /// `ghostty-vt` producer wires this yet; callers not sourcing it should
    /// pass `false`.
    pub password_input: bool,
    /// The terminal-requested visual style (DECSCUSR).
    pub visual_style: ghostty_vt::screen::cursor::CursorStyle,
}
...
mod tests {
    ...
    fn default_uses_configured_style() {
        ...
        term.modes
            .set(ghostty_vt::modes::Mode::CursorBlinking, true);
    ...
    fn blinking_disabled() {
        ...
        term.screen_mut().cursor.cursor_style = CursorStyle::Bar;
        term.modes
            .set(ghostty_vt::modes::Mode::CursorBlinking, false);
    ...
    fn always_block_with_preedit() {
        ...
        // that as `viewport: None` (see `CursorState` docs — `ghostty-vt`
