// Ported from Ghostty's `src/renderer/shaders/shaders.metal` (commit
// `2da015cd6`), first-pixels subset only (chunk R3, docs/plans/m3-first-pixels.md
// decision 6: "port MSL verbatim"). Kept verbatim including upstream comments;
...
//
// Buffer index convention (frozen, `crates/ghostty-renderer/src/wire.rs`):
// index 0 = vertex/instance data, index 1 = uniforms, 2+ = extras (e.g. the
// per-cell background-color array consumed by `cell_bg_fragment` and
// `cell_text_vertex`).
//
// `Uniforms` below must match `wire::Uniforms` bit-for-bit (size 144, align
// 16; see wire.rs's frozen offset tests) and `CellTextVertexIn` must match
// `wire::CellText` (size 32, align 8) — this file does not redeclare those
// invariants; `shaders/mod.rs`'s `PIPELINE_DESCRIPTIONS` table and its tests
// cross-reference the wire.rs offsets directly.
