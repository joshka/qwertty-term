//! C ABI over the ghostty-rs engine (M5 FFI spike scope).
//!
...
//! key round-trip through [`ghostty_input`], raw PTY-byte write, a screen-text
//! dump (pre-M3 stand-in for `surface_draw`), and an OSC 52 clipboard callback.
//!
//! # Design notes
//!
//! - **Header is generated.** `cbindgen` produces `include/ghostty_rs.h` from
...
//!   UB, so a caught panic becomes [`GHOSTTY_RS_PANIC`] (or a null handle for
...
use ghostty_input::key::{Action as InputAction, Key, KeyEvent};
...
use ghostty_vt::terminal::{Options as TerminalOptions, Terminal};
...
pub const GHOSTTY_RS_SUCCESS: i32 = GhosttyRsResult::Success as i32;
/// See [`GhosttyRsResult::Panic`].
pub const GHOSTTY_RS_PANIC: i32 = GhosttyRsResult::Panic as i32;
...
pub type GhosttyRsWakeupCb = Option<extern "C" fn(userdata: *mut c_void)>;
...
pub type GhosttyRsWriteClipboardCb =
    Option<extern "C" fn(userdata: *mut c_void, kind: GhosttyRsClipboard, data: *const c_char)>;
...
#[repr(C)]
#[derive(Debug, Clone, Copy)]
pub struct GhosttyRsSurfaceConfig {
...
// ---------------------------------------------------------------------------
// Input key event (mirror ghostty_input_key_s).
// ---------------------------------------------------------------------------

/// Key action, mirroring upstream `ghostty_input_action_e`.
#[repr(C)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum GhosttyRsInputAction {
    /// Key released.
    Release = 0,
    /// Key pressed.
    Press = 1,
...
#[repr(C)]
#[derive(Debug, Clone, Copy)]
pub struct GhosttyRsInputKey {
    /// Press / release / repeat.
    pub action: GhosttyRsInputAction,
    /// Active modifiers.
    pub mods: GhosttyRsInputMods,
...
pub struct FfiApp {
    runtime: GhosttyRsRuntimeConfig,
}
...
impl FfiSurface {
    ...
    fn fire_clipboard(&self, kind: u8, data: &str) {
        ...
        let selection = if kind == b'p' {
            GhosttyRsClipboard::Selection
        } else {
            GhosttyRsClipboard::Standard
        };
...
/// Run `f`, converting any panic into `GHOSTTY_RS_PANIC`. Used by fns that
/// return a result code.
fn guard_result(f: impl FnOnce() -> GhosttyRsResult) -> GhosttyRsResult {
    match catch_unwind(AssertUnwindSafe(f)) {
        Ok(r) => r,
        Err(_) => GhosttyRsResult::Panic,
    }
}
...
#[unsafe(no_mangle)]
pub extern "C" fn ghostty_rs_init() -> GhosttyRsResult {
    guard_result(|| GhosttyRsResult::Success)
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghostty_rs_app_new(config: *const GhosttyRsRuntimeConfig) -> *mut FfiApp {
    guard_ptr(|| {
        if config.is_null() {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghostty_rs_app_tick(app: *mut FfiApp) {
    guard_void(|| {
        let _ = app;
    });
}
...
/// no-op.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghostty_rs_app_free(app: *mut FfiApp) {
    guard_void(|| {
        if !app.is_null() {
            // SAFETY: `app` came from `Box::into_raw` in `ghostty_rs_app_new`
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghostty_rs_surface_new(
    app: *mut FfiApp,
    config: *const GhosttyRsSurfaceConfig,
) -> *mut FfiSurface {
    guard_ptr(|| {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghostty_rs_surface_free(surface: *mut FfiSurface) {
    guard_void(|| {
        if !surface.is_null() {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghostty_rs_surface_write_pty_bytes(
    ...
    guard_result(|| {
        if surface.is_null() || (data.is_null() && len != 0) {
            return GhosttyRsResult::NullArgument;
        }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghostty_rs_surface_key(
    surface: *mut FfiSurface,
    event: GhosttyRsInputKey,
) -> GhosttyRsResult {
    guard_result(|| {
        if surface.is_null() {
            return GhosttyRsResult::NullArgument;
        ...
        if event.composing {
            ...
            return GhosttyRsResult::Success;
        }
        ...
        let bytes = if is_plain_text {
            ...
            let key_event = KeyEvent {
                action: match event.action {
                    GhosttyRsInputAction::Release => InputAction::Release,
                    GhosttyRsInputAction::Press => InputAction::Press,
                    GhosttyRsInputAction::Repeat => InputAction::Repeat,
                },
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghostty_rs_surface_read_text(
    surface: *mut FfiSurface,
    buf: *mut c_char,
    buf_len: usize,
    out_len: *mut usize,
    ...
    guard_result(|| {
        ...
        if buf_len < needed {
            return GhosttyRsResult::OutOfSpace;
        }
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn ghostty_rs_surface_take_pty_reply(
    surface: *mut FfiSurface,
    buf: *mut u8,
    buf_len: usize,
    out_len: *mut usize,
    ...
    guard_result(|| {
        ...
        if buf.is_null() {
            return GhosttyRsResult::Success; // size query, no consume
        }
        if buf_len < needed {
            return GhosttyRsResult::OutOfSpace;
        }
...
/// queue is left for [`ghostty_rs_surface_take_pty_reply`] to drain.
...
/// Decode the C mods bitfield into a `ghostty_input::Mods`.
fn decode_mods(mods: GhosttyRsInputMods) -> Mods {
    Mods {
