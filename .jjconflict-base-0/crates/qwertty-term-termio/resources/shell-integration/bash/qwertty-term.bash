...
if [ -n "$QWERTTY_TERM_BASH_INJECT" ]; then
  ...
  builtin declare __ghostty_bash_flags="$QWERTTY_TERM_BASH_INJECT"
  builtin unset ENV QWERTTY_TERM_BASH_INJECT

  # Restore an existing ENV that was replaced by the shell integration code.
  if [[ -n "$QWERTTY_TERM_BASH_ENV" ]]; then
    builtin export ENV=$QWERTTY_TERM_BASH_ENV
    builtin unset QWERTTY_TERM_BASH_ENV
  fi
  ...
  if [[ -n "$QWERTTY_TERM_BASH_UNEXPORT_HISTFILE" ]]; then
    builtin export -n HISTFILE
    builtin unset QWERTTY_TERM_BASH_UNEXPORT_HISTFILE
  fi
  ...
  if builtin shopt -q login_shell; then
    ...
    if [[ $__ghostty_bash_flags != *"--norc"* ]]; then
      ...
      if [[ -z "$QWERTTY_TERM_BASH_RCFILE" ]]; then QWERTTY_TERM_BASH_RCFILE="$HOME/.bashrc"; fi
      [ -r "$QWERTTY_TERM_BASH_RCFILE" ] && builtin source "$QWERTTY_TERM_BASH_RCFILE"
    fi
  fi

  builtin unset __ghostty_rcfile
  builtin unset __ghostty_bash_flags
  builtin unset QWERTTY_TERM_BASH_RCFILE
fi

# Add Ghostty binary to PATH if the path feature is enabled
if [[ "$QWERTTY_TERM_SHELL_FEATURES" == *"path"* && -n "$QWERTTY_TERM_BIN_DIR" ]]; then
  if [[ ":$PATH:" != *":$QWERTTY_TERM_BIN_DIR:"* ]]; then
    export PATH="$PATH:$QWERTTY_TERM_BIN_DIR"
  fi
fi

# Sudo
if [[ "$QWERTTY_TERM_SHELL_FEATURES" == *"sudo"* && -n "$TERMINFO" ]]; then
...
if [[ "$QWERTTY_TERM_SHELL_FEATURES" == *ssh-* ]]; then
  function ssh() {
    builtin local -a flags
    flags=()
    [[ "$QWERTTY_TERM_SHELL_FEATURES" != *ssh-env* ]] && flags+=(--forward-env=false)
    [[ "$QWERTTY_TERM_SHELL_FEATURES" != *ssh-terminfo* ]] && flags+=(--terminfo=false)
    "$QWERTTY_TERM_BIN_DIR/qwertty-term" +ssh "${flags[@]}" -- "$@"
  }
fi
...
function __ghostty_precmd() {
  ...
  if test "$_ghostty_executing" != "0"; then
    _QWERTTY_TERM_SAVE_PS1="$PS1"
    _QWERTTY_TERM_SAVE_PS2="$PS2"
    ...
    # Cursor
    if [[ "$QWERTTY_TERM_SHELL_FEATURES" == *"cursor"* ]]; then
      builtin local cursor=5  # blinking bar
      [[ "$QWERTTY_TERM_SHELL_FEATURES" == *"cursor:steady"* ]] && cursor=6  # steady bar
    ...
    if [[ "$QWERTTY_TERM_SHELL_FEATURES" == *"title"* ]]; then
      PS1=$PS1'\[\e]2;\w\a\]'
    fi
  fi
...
function __ghostty_preexec() {
  ...
  PS1="$_QWERTTY_TERM_SAVE_PS1"
  PS2="$_QWERTTY_TERM_SAVE_PS2"

  # Title (current command)
  if [[ -n $cmd && "$QWERTTY_TERM_SHELL_FEATURES" == *"title"* ]]; then
