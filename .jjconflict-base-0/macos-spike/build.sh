#!/usr/bin/env bash
# Build + run the ghostty-rs FFI Swift driver.
#
...
echo "==> Building ghostty-ffi staticlib ($PROFILE)"
if [ "$PROFILE" = "release" ]; then
  cargo build --manifest-path "$REPO_ROOT/Cargo.toml" -p ghostty-ffi --release
else
  cargo build --manifest-path "$REPO_ROOT/Cargo.toml" -p ghostty-ffi
fi

LIB_DIR="$REPO_ROOT/target/$PROFILE"
OUT="$SCRIPT_DIR/ghostty-rs-spike"

echo "==> Compiling Swift driver"
# -I imports the clang module map (CGhosttyRs -> the cbindgen header).
