...
- **Ship via the PR pipeline** (`docs/threads/README.md`): `jj describe` → push a bookmark
  (`jj git push --bookmark <id>/<feature>`) → `gh pr create` → merge (which advances
  `main`). Small doc-only changes may land direct to main. `trunk()` (== `main`) is the
  integration point; keep it green.
