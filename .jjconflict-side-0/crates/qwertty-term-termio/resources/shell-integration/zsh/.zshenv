...
# environment and should not be sourced manually (unlike qwertty-term-integration).
#
...
# Restore the original ZDOTDIR value if QWERTTY_TERM_ZSH_ZDOTDIR is set.
# Otherwise, unset the ZDOTDIR that was set during shell injection.
if [[ -n "${QWERTTY_TERM_ZSH_ZDOTDIR+X}" ]]; then
    'builtin' 'export' ZDOTDIR="$QWERTTY_TERM_ZSH_ZDOTDIR"
    'builtin' 'unset' 'QWERTTY_TERM_ZSH_ZDOTDIR'
else
...
# Use try-always to have the right error code.
{
    ...
    #
    # Source the user's .zshenv before sourcing qwertty-term-integration because the
    # former might set fpath and other things without which qwertty-term-integration
    # won't work.
    #
    ...
    if [[ -o 'interactive' ]]; then
        ...
        'builtin' 'typeset' _ghostty_file="${${(%):-%x}:A:h}"/qwertty-term-integration
