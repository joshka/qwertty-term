...
//! `qwertty-term-sprite`), reserves an atlas region, uploads the bitmap, and caches
...
pub struct Grid {
    ...
    sprite_metrics: qwertty_term_sprite::Metrics,
}
...
impl Grid {
    ...
    fn render_sprite(&mut self, cp: u32) -> Result<CachedGlyph, Error> {
        let glyph =
            qwertty_term_sprite::render(cp, &self.sprite_metrics).ok_or(Error::SpriteMissing)?;
...
/// Derive `qwertty_term_sprite::Metrics` from font [`Metrics`].
///
...
fn sprite_metrics_from(m: &Metrics) -> qwertty_term_sprite::Metrics {
    qwertty_term_sprite::Metrics {
