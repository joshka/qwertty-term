...
//!   2. `$GHOSTTY_RS_THEMES_DIR/<name>` if set, else a hardcoded fallback
...
impl ThemeColors {
    ...
    /// Build the `ghostty-vt` startup [`Colors`] this theme implies: the full
    /// 256-entry palette (defaults, with any theme overrides applied) plus
    /// OSC-10/11-equivalent default fg/bg/cursor. `ghostty-vt`'s `Colors` /
...
/// hardcoded checkout path). Returns `None` (falling back to `ghostty-vt`'s
...
";
    ...
    fn resolves_absolute_path_as_is() {
        let dir = std::env::temp_dir().join("ghostty-app-theme-test-absolute");
